...
<body>
	<div class="container">
		...
		<a href="list.jsp?pt=all" style='${(pt=="all")?style4:""}' class="btn btn-default">전체</a>
		<table class="table">
			<tr>
				<th>번호</th>
				<th>제목</th>
			...
			<c:if test="${!empty list }">
				<c:forEach items="${list }" var="vo">
					<tr onclick="document.location='view.jsp?no=${vo.no}&page=${pageObject.page }&perPageNum=${pageObject.perPageNum }'"
						class="dataRow">
						<td>${vo.no}</td>
						<td>${vo.title}</td>
						<td>${vo.startDate}</td>
			...
			<tr>
				<td colspan="5"><pageNav:pageNav listURI="list.jsp" pageObject="${pageObject }" query="&pt=${pt }"></pageNav:pageNav></td>
			</tr>
